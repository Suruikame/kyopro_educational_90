...
func main() {
    ...
    for i in 0..<N {
        let inputs = readInts()
        A.append((inputs[0], inputs[1]))
        flow.addEdge(0, i+1)
    }
    for i in 0..<N {
        let inputs = readInts()
        B.append((inputs[0], inputs[1]))
        Bdict[inputs[0] * 1_000_000_001 + inputs[1]] = i
        flow.addEdge(N+i+1, 2*N+1)
    }
    let dys = [-1, -1, 0, 1, 1, 1, 0, -1], dxs = [0, -1, -1, -1, 0, 1, 1, 1]
    for (i, (x, y)) in A.enumerated() {
        for (dx, dy) in zip(dys, dxs) {
            let (nx, ny) = (x+T*dx, y+T*dy)
            if nx >= 1_000_000_000 || ny >= 1_000_000_000 || nx < 0 || ny < 0 {
                continue
            }
            if let j = Bdict[nx * 1_000_000_001 + ny] {
                flow.addEdge(i+1, N+j+1)
            }
        }
    }

    //dinic法を適用して、流量が1の辺を探す
    if flow.dinic(0, 2*N+1) == N {
        print("Yes")
        var ans = ""
        for from in 0..<N {
            for edge in flow.graph[from + 1] {
                if edge.capacity == 0 {
                    let to = edge.to - N - 1
                    let (x, y) = A[from]
                    let (nx, ny) = B[to]
                    switch ((nx - x).signum(), (ny - y).signum()) {
                        case (-1, 0): ans += "5 "
                        case (-1, -1): ans += "6 "
                        case (0, -1): ans += "7 "
                        case (1, -1): ans += "8 "
