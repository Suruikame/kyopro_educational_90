...
func main(){
    ...
    var lazytree = LazySegmentTree<Int, Int>(fxx: {max($0, $1)}, fxm: {$1 == -(1<<60) ? $0 : $1}, fmm: {max($0, $1)}, ex: -(1<<60), em: -(1<<60))
    lazytree.build(Array(repeating: 0, count: W))
    for _ in 0..<N {
        let inputs = readInts()
        let (L, R) = (inputs[0], inputs[1])
        let newHeight = lazytree.query(L, R+1) + 1
        print(newHeight)
        lazytree.operateRange(L, R+1, newHeight)
    }
}
